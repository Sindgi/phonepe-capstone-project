...
if SELECT == "Contact":
    ...
    with col2:
        ...
        st.write("---")
        st.subheader("Mail: " + mail)
    st.write("#")
    cols = st.columns(len(social_media))
    for platform, link in social_media.items():
        cols.write(f"[{platform}]({link})")
...
#creating functions for query search in sqlite to get the data
    ...
    def transaction_state(_state):
        cursor.execute(f"SELECT state,year,quarter,entity_name,count(*)as c_ount,sum(a_mount) as amount FROM top_transaction WHERE state = '{_state}' GROUP BY state,year,quarter,entity_name")
        df = pd.DataFrame(cursor.fetchall(), columns=['state', 'year',"quarter", 'entity_name', 'c_ount', 'amount'])
        return df
    def transaction_year(_state,_year):
        cursor.execute(f"SELECT state,year,quarter,entity_name,count(*)as c_ount,sum(a_mount) as amount FROM top_transaction FROM top_transaction WHERE year = '{_year}' AND state = '{_state}' GROUP BY state,year,quarter,entity_name")
        df = pd.DataFrame(cursor.fetchall(), columns=['state', 'year',"quarter", 'entity_name', 'c_ount', 'amount'])
        return df
    def transaction_quarter(_state,_year,_quarter):
        cursor.execute(f"SELECT state,year,quarter,entity_name,count(*)as c_ount,sum(a_mount) as amount  FROM top_transaction WHERE year = '{_year}' AND quarter = '{_quarter}' AND state = '{_state}' GROUP BY state,year,quarter.entity_name")
        df = pd.DataFrame(cursor.fetchall(), columns=['state', 'year','quarter', 'entity_name', 'c_ount', 'amount'])
        return df
